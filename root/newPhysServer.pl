...
# you choose an IP for it.
#
# This script adds the records to the Prime DB (TODO: document the DB schema)
# then starts the installation via Cobbler.
#
# Run this script, then power on the new blank server. 
# It will PXE boot from cobbler, follow the kickstart, 
...
# There is a bios command that can erase and reset a server. 
...
if ($#ARGV != 0 ) {
    print "usage: newPhysServer.pl IP \n";
    print "   eg: newPhysServer.pl 109.187.243.86 \n";
    exit 0;
}
...
# get id of most recently installed server.
# Server names are assigned in sequence odin1 odin2 ..
# data (dummy, dummy, odin2)
...
# TODO config prime db IP addre here?
# at DNS server, edit
#   /etc/bind/zones/external/db.example.net
# (this might be optional, the Domain name might not be needed)

# TODO: automate this, edit 
# /etc/puppet/manifests/nodes.pp 
# IE
#  case $::hostname {
#        'odin': { $ipaddr = "109.187.243.86" }
#  }
...
$commandline = "sed -i[sav] -e '/$new_server_name.*ipaddr/s/109.187.243.[0-9]*/$new_external_IP/' "
    . " /etc/puppet/manifests/nodes.pp";
`$commandline`;
